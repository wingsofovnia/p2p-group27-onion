 ...
 */
@Slf4j
public class TunnelConnectHandler extends MessageToMessageDecoder<Pair<TunnelId, TunnelConnect>> {
    ...
    @Override
    protected void decode(ChannelHandlerContext ctx, Pair<TunnelId, TunnelConnect> tunnelIdConnect, List<Object> out)
            throws Exception {
        ...
        clientChannelFactory.connect(connect.socketDestination()).thenAccept(channel -> {
            ...
            val extendMsg = new TunnelExtendMessage(tunnelId, connect.requestId(), connect.sourceKey(), connect.handshake());
