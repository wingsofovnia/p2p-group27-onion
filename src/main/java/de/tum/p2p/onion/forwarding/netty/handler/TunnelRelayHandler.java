 ...
 */
@Slf4j
public class TunnelRelayHandler extends MessageToMessageDecoder<TunnelRelayMessage> {
    ...
    @Override
    protected void decode(ChannelHandlerContext ctx, TunnelRelayMessage relay, List<Object> out) throws Exception {
        val tunnelId = relay.tunnelId();
        val relayPayload = relay.payload();

        val deciphertext = onionAuth.decrypt(relayPayload, routingContext.sessionId(tunnelId)).join();
        ...
        switch (MessageType.fromBytes(rawRelayPayload)) {
            ...
            case ONION_TUNNEL_DATUM:
                // FIXME: Netty doesn't recognize
                out.add(new javafx.util.Pair<>(tunnelId, TunnelDatum.fromBytes(rawRelayPayload)));
                break;

            case ONION_TUNNEL_CONNECT:
                out.add(Pair.of(tunnelId, TunnelConnect.fromBytes(rawRelayPayload)));
                break;
